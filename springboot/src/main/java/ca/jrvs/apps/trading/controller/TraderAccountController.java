...
public class TraderAccountController {
    ...
    @ApiOperation(value = "Get all traders", notes = "fetch all traders in database and returns")
    ...
    @ApiOperation(value = "Get all accounts", notes = "fetch all accounts in database an returns")
    @GetMapping(path = "/traders/accounts", produces = {MediaType.APPLICATION_JSON_UTF8_VALUE})
    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
    public Iterable<AccountEntity> accounts() {
        return this.service.getAllAccounts();
    }

    @ApiOperation(
            value = "Create trader and an account",
            notes = "traderId and accountId are auto generated by database, they should be mostly the same"
    )
    @GetMapping(
            path = "/firstname/{firstname}/lastname/{lastname}/dob/{dob}/country/{country}/email/{email}/gender/{gender}",
    ...
    @ApiOperation(value = "Update trader")
    @PutMapping(
            path = "update/id/{id}/firstname/{firstname}/lastname/{lastname}/country/{country}/email/{email}/gender/{gender}",
            produces = {MediaType.APPLICATION_JSON_UTF8_VALUE}
    )
    @ResponseStatus(HttpStatus.CREATED)
    @ResponseBody
    public TraderAccountView updateTrader(
        ...
        try {
            TraderEntity savedTraderEntity = new TraderEntity();
            savedTraderEntity.setId(Integer.valueOf(id));
            savedTraderEntity.setFirstName(firstname);
            savedTraderEntity.setLastName(lastname);
            savedTraderEntity.setCountry(country);
            savedTraderEntity.setEmail(email);
            savedTraderEntity.setGender(gender);
            savedTraderEntity = this.service.updateTraderEntity(savedTraderEntity);
            return this.service.getTraderAndAccount(savedTraderEntity.getId());
        }catch (Exception e) {
            throw ResponseExeptionUtil.getResponseStatusExecption(e);
        }
    }
    ...
    @ApiOperation(
            ...
            notes = "Delete a trader if its account amount is 0 and no open positions."
    )
    @ApiResponses(value = {
            @ApiResponse(
                    code = 400,
                    message = "Unable to delete the trader"
            ),
            @ApiResponse(code = 404, message = "trade id not found")
    })
    @DeleteMapping(
            path = "/delete/traderid/{traderId}",
            produces = {MediaType.APPLICATION_JSON_UTF8_VALUE}
    )
    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
